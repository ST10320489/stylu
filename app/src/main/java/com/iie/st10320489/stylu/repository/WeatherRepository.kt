    ...
    suspend fun getWeeklyForecast(
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                val dailyWeather =
                    (0 until kotlin.comparisons.minOf(7, response.daily.time.size)).map { index ->
                        ...
                        Log.d(
                            TAG,
                            "Day ${index + 1}: ${day.day} - ${day.minTemp}°C to ${day.maxTemp}°C - Code: $weatherCode -> ${day.condition}"
                        )
                        day
                    }
